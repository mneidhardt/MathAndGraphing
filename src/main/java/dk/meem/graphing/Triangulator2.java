...
public class Triangulator2 {
	...
	*/
	...
	public int Triangulate ( int nv, Point pxyz[], Triangle v[] )	{
		...
		edges = new Edge[emax];
		for (int ie=0; ie<emax; ie++) edges[ie] = new Edge();
		...
		*/
		...
		for (int i=1;i<nv;i++)
		{
			if (pxyz[i].getX() < xmin) xmin = pxyz[i].getX();
			if (pxyz[i].getX() > xmax) xmax = pxyz[i].getX();
			if (pxyz[i].getY() < ymin) ymin = pxyz[i].getY();
			if (pxyz[i].getY() > ymax) ymax = pxyz[i].getY();
		}
		dx = xmax - xmin;
		dy = ymax - ymin;
		dmax = (dx > dy) ? dx : dy;
		...
		/*
			...
			the triangle list.
		*/
		pxyz[nv+0].setX(xmid - 2.0 * dmax);
		pxyz[nv+0].setY(ymid - dmax);
		pxyz[nv+0].setZ(0.0);
		pxyz[nv+1].setX(xmid);
		pxyz[nv+1].setY(ymid + 2.0 * dmax);
		pxyz[nv+1].setZ(0.0);
		pxyz[nv+2].setX(xmid + 2.0 * dmax);
		pxyz[nv+2].setY(ymid - dmax);
		pxyz[nv+2].setZ(0.0);
		v[0].p1 = nv;
		v[0].p2 = nv+1;
		v[0].p3 = nv+2;
	...
	public Point[] init(int nv) {
		Point[] points = new Point[ nv+3 ];
		
		for (int i=0; i<points.length; i++)
			points[i] = new Point( i*4.0, 400.0 * Math.random(), 0.0 );
		
		this.triangles = new Triangle[ nv*3 ];
		
		for (int i=0; i<triangles.length; i++) {
			triangles[i] = new Triangle();
		}
		
		this.ntri = this.Triangulate(nv, points, triangles);
